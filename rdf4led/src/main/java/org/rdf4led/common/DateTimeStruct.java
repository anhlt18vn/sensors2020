...
public class DateTimeStruct {
  ...
  public String toString() {
    ...
    if (xsdDateTime) x = x + "T" + hour + tSep + minute + tSep + second;
    if (neg != null) x = neg + x;
    if (timezone != null) x = x + timezone;
    return x;
  }
  ...
  private static DateTimeStruct _parseYMD(
      String str, boolean month, boolean day, boolean includeTime) {
  ...
  private static int _parseTime(DateTimeStruct struct, int idx, String str) {
    ...
    if (idx < str.length() && str.charAt(idx) == '.') {
      ...
      if (idx == idx2) throw new DateTimeParseException("Bad time part");
  ...
  private static int _parseTimezone(DateTimeStruct struct, String str, int idx) {
    ...
    if (str.charAt(idx) == 'Z') {
      ...
      else if (str.charAt(idx) == '-') sb.append('-');
      else throw new DateTimeParseException("Bad timezone");
      idx += 1;
  ...
  private static void check(String string, int idx, char x) {
    if (string.length() <= idx || string.charAt(idx) != x)
      throw new DateTimeParseException("Expected: " + x + " at storage " + idx);
  }
}
