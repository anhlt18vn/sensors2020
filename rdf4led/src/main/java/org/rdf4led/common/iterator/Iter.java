...
public class Iter<T> implements Iterable<T>, Iterator<T> {
  ...
  //            private Set<T> org.rdf4led.common.data.incremental.acc = null ;
  //
  //            @Override
  //            public void accumulate(T item) {
  //                org.rdf4led.common.data.incremental.acc.add(item) ;
  //            }
  //
  //            @Override
  //            public Set<T> get() {
  //                return org.rdf4led.common.data.incremental.acc ;
  //            }
  //
  //            @Override
  //            public void start() {
  //                org.rdf4led.common.data.incremental.acc = new HashSet<>() ;
  //            }
  //
  //            @Override
  //            public void finish() {}
  ...
  //            private List<T> org.rdf4led.common.data.incremental.acc = null ;
  //
  //            @Override
  //            public void accumulate(T item) {
  //                org.rdf4led.common.data.incremental.acc.add(item) ;
  ...
  //            @Override
  //            public void start() {
  //                org.rdf4led.common.data.incremental.acc = new ArrayList<>() ;
  //            }
  //
  //            @Override
  //            public void finish() {}
  ...
  /**
   * Create another iterator without risk of concurrent modification exceptions. This materializes
   * the input iterator.
   */
  ...
  public static <T> Iterator<T> notFilter(
      final Iterator<? extends T> stream, final Filter<T> filter) {
  ...
  public static <T> boolean every(Iterable<? extends T> stream, Filter<T> filter) {
    for (T item : stream) if (!filter.accept(item)) return false;
    return true;
  }

  /**
   * Return true if every element of stream passes the filter (reads the stream until the first
   * element not passing the filter)
   */
  ...
  /**
   * Return true if every element of stream passes the filter (reads the stream until the first
   * element passing the filter)
   */
  public static <T> boolean some(Iterable<? extends T> stream, Filter<T> filter) {
    for (T item : stream) if (filter.accept(item)) return true;
    return false;
  }

  /**
   * Return true if one or more elements of stream passes the filter (reads the stream to first
   * element passing the filter)
   */
  public static <T> boolean some(Iterator<? extends T> stream, Filter<T> filter) {
    for (; stream.hasNext(); ) {
      T item = stream.next();
      if (filter.accept(item)) return true;
    }
    return false;
  }
  ...
  public static <T, R> Iterator<R> map(
      final Iterator<? extends T> stream, final Transform<T, R> converter) {
    final Iterator<R> iter =
        new Iterator<R>() {
          @Override
          public boolean hasNext() {
  ...
  /**
   * Projects each element of a sequence to an Iterator&lt;R&gt; and flattens the resulting
   * sequences into one sequence.
   */
//  public static <T, R> Iterator<R> mapMany(
//      final Iterator<? extends T> stream, final Transform<? super T, Iterator<R>> converter) {
//    final Iterator<R> iter =
//        new Iterator<R>() {
//
...
//  }
  ...
  //    public static <T, R> List<R> mapMany(List<? extends T> list, Transform<T, Iterator<R>>
  // converter) {
  ...
  /** Apply an action to everything in stream, yielding a stream of the same items */
  //    public static <T> Iterator<T> operate(final Iterator<? extends T> stream, final Action<T>
  // action) {
  ...
  /** Print an iterator as it gets used - this adds a printing wrapper */
  //    public static <T> Iterator<T> printWrapper(final PrintStream out, final Iterator<? extends
  // T> stream) {
  ...
  /**
   * Join two iteratables If there, potentially, going to be many iterators, it is better to create
   * an {@link IteratorConcat} explicitly and add each iterator.
   */
  //    public static <T> Iterator<T> append(Iterable<? extends T> iter1, Iterable<? extends T>
  // iter2) {
  //        return IteratorCons.create(iterator(iter1), iterator(iter2)) ;
  //    }

  /**
   * Join two iterator If there, potentially, going to be many iterators, it is better to create an
   * {@link } explicitly and add each iterator.
   */
  //    public static <T> Iterator<T> append(Iterator<? extends T> iter1, Iterator<? extends T>
  // iter2) {
  ...
  public static <T> List<T> take(Iterator<T> iter, int N) {
    ...
    for (; iter.hasNext(); ) x.add(iter.next());
    return x;
  }
  ...
  /**
   * Count the iterable - many iterable objects have a .size() operation which should be used in
   * preference to this explicit counting operation
   */
  ...
  /**
   * Print an iterator to stdout, return a copy of the iterator. Printing occurs now. See {@link
   * #debug} for an operation to print as the iterator is used.
   */
  ...
  /**
   * Print an iterator to stdout, return a copy of the iterator. Printing occurs when the returned
   * iterator is used
   */
  ...
  /**
   * Print an iterator to stdout, return a copy of the iterator. Printing occurs when the iterator
   * is used. See {@link } for an operation to print now.
   */
...
//  }
  ...
  public static <T> Iter<T> iter(Iterator<T> iterator) {
    if (iterator instanceof Iter<?>) return (Iter<T>) iterator;
    return new Iter<>(iterator);
  }

  public static <T> Iter<T> iter(Iterable<T> iterable) {
    if (iterable instanceof Iter<?>) return (Iter<T>) iterable;
  ...
  public static <T> int lastIndex(Iterator<T> iter, Filter<T> filter) {
    ...
    for (int idx = 0; iter.hasNext(); idx++) {
      T t = iter.next();
      if (filter.accept(t)) location = idx;
    }
    return location;
  }
  ...
  /**
   * Join on an iterator. If there are going to be many iterators, uit is better to create an {@link
   * IteratorConcat} and <tt>.add</tt> each iterator. The overheads are much lower.
   */
