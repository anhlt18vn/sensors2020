...
/**
 * Path evaluator that produces duplicates. This is NOT SPARQL semantics. This class exists for
 * experimentation. It is written to get the right results - not necessarily with maximum
 * efficiency.
 */
final class PathEngineN<Node> extends PathEngine<Node> {
  private boolean forwardMode;

  public PathEngineN(Graph<Node> graph, boolean forward) {
    super(graph, null);
  ...
  protected void doSeq(
    ...
    for (Node n : iter) eval(part2, n, output);
  }
  ...
  private void doOne(Path<Node> path, Node node, Collection<Node> output) {
