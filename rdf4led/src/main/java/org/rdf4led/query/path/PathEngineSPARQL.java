...
public class PathEngineSPARQL<Node> extends PathEngine<Node> {
  private boolean forwardMode;

  public PathEngineSPARQL(Graph graph, QueryContext<Node> queryContext) {
    this(graph, true, queryContext);
  }

  /* package */ PathEngineSPARQL(Graph graph, boolean forward, QueryContext<Node> queryContext) {
    super(graph, queryContext);
  ...
  protected void doSeq(
    ...
    Path<Node> part1 = forwardMode ? pathStepLeft : pathStepRight;

    Path<Node> part2 = forwardMode ? pathStepRight : pathStepLeft;
  ...
  @Override
  protected void doZeroOrMore(Path<Node> pathStep, Node node, Collection<Node> output) {
    // Reuse "output"
  ...
  @Override
  protected void doOneOrMore(Path<Node> pathStep, Node node, Collection<Node> output) {
    // Reuse "output"
    Collection<Node> visited = new LinkedList<>(); // new
    // HashSet<Node>() ;
  ...
  private void ALP_N(
      Node node, Path<Node> path, Collection<Node> visited, Collection<Node> output) {
    if (visited.contains(node)) return;
  ...
  protected void doMultiLengthPath(
    ...
    // TODO Rewrite
    Path<Node> p1 = pathStep;
    Path<Node> p2 = new P_Mod<Node>(pathStep, min2, max2);
  ...
  protected void doFixedLengthPath(
    ...
    P_FixedLength<Node> nextPath = new P_FixedLength(pathStep, count2);
