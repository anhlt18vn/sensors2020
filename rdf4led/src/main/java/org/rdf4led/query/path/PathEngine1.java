...
/**
 * Path evaluation visitor that provide distinct nodes visited only. This is NOT SPARQL semantics.
 * This class exists for experimentation. It is written to get the right results - not necessarily
 * with maximum efficiency.
 */
final class PathEngine1<Node> extends PathEngine<Node> {
  private boolean forwardMode;

  public PathEngine1(Graph<Node> graph, boolean forward) {
    super(graph, null);
  ...
  protected void doSeq(
    ...
    for (Node n : nodes) eval(part2, n, nodes2);
    output.addAll(nodes2);
  }

  // Can use .addAll if collector is set-like.
  private static <Node> void fillUnique(Iterator<Node> nodes, Collection<Node> acc) {
    for (; nodes.hasNext(); ) {
      Node n = nodes.next();
  ...
  @Override
  protected void doMultiLengthPath(
      Path<Node> pathStep, Node node, long min1, long max1, Collection<Node> output) {
    ...
    if (min1 > 0) doFixedLengthPath(pathStep, node, min1, collectStartPoints);
    else collectStartPoints.add(node);
  ...
  // {0,length}
  protected void doMultiLengthPath(
      Path pathStep, Node node, long length, Collection<Node> visited, Collection<Node> output) {
  ...
  @Override
  protected void doFixedLengthPath(
    ...
    // Loop, not recurse.
    Iter<Node> iter = eval(pathStep, node);
    for (Node n : iter) doFixedLengthPath(pathStep, n, fixedLength - 1, output);
    return;
  }

  @Override
  protected void doZeroOrMore(Path<Node> path, Node node, Collection<Node> output) {
    // Reuse "output"
  ...
  @Override
  protected void doOneOrMore(Path<Node> path, Node node, Collection<Node> output) {
    // Reuse "output"
    Collection<Node> visited = new LinkedList<>(); // new
    // HashSet<Node>() ;
  ...
  @Override
  protected void doNegatedPropertySet(
  ...
  @Override
  protected void doZeroOrOne(Path<Node> pathStep, Node node, Collection<Node> output) {
    eval(pathStep, node, output);
    if (!output.contains(node)) output.add(node);
  }

  @Override
  protected void doZero(Path<Node> path, Node node, Collection<Node> output) {
    if (!output.contains(node)) output.add(node);
  }
}
