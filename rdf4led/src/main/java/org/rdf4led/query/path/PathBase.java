...
public abstract class PathBase<Node> implements Path<Node> {
  ...
  @Override
  public abstract boolean equalTo(Path<Node> path2); // , NodeIsomorphismMap isoMap) ;

  @Override
  public final boolean equals(Object path2) {
    if (this == path2) return true;

    if (!(path2 instanceof Path)) return false;

    return equalTo((Path<Node>) path2);
  }
}
