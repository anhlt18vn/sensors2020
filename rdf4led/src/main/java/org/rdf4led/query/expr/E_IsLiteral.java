 ...
 * <p>10 Sep 2015
 */
public class E_IsLiteral<Node> extends ExprFunction1<Node> {
  public E_IsLiteral(Expr<Node> expr, QueryContext<Node> queryContext) {
    super(expr, queryContext);
  }

  @Override
  public Node eval(Node v) {
    if (queryContext.dictionary().isLiteral(v)) {
  ...
  public Expr<Node> copy(Expr<Node> expr) {
    return new E_IsLiteral<Node>(expr, queryContext);
  }
}
