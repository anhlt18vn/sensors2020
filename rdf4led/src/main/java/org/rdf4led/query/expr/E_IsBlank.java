 ...
 * <p>10 Sep 2015
 */
public class E_IsBlank<Node> extends ExprFunction1<Node> {

  public E_IsBlank(Expr<Node> expr, QueryContext<Node> queryContext) {
    super(expr, queryContext);
  }

  @Override
  public Node eval(Node v) {
    if (queryContext.dictionary().isBNode(v)) {
  ...
  public Expr<Node> copy(Expr<Node> expr) {
    return new E_IsBlank<Node>(expr, queryContext);
  }
}
