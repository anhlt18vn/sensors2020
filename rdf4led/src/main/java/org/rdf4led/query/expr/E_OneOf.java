 ...
 * <p>10 Sep 2015
 */
public class E_OneOf<Node> extends E_OneOfBase<Node> {
  // private static final String printName = "in" ;

  public E_OneOf(Expr<Node> expr, ExprList<Node> args, QueryContext<Node> queryContext) {
    super(expr, args, queryContext);
  }

  protected E_OneOf(ExprList<Node> args, QueryContext<Node> queryContext) {
    super(args, queryContext);
  }

  @Override
  public Node evalSpecial(Mapping<Node> mapping) {
    boolean b = super.evalOneOf(mapping);
  ...
  protected Expr<Node> copy(ExprList<Node> newArgs) {
    return new E_OneOf<>(newArgs, queryContext);
  }
}
