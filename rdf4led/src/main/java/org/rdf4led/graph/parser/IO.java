...
public class IO {
  ...
  /**
   * Open an input stream to a file. If the filename is null or "-", return System.in If the
   * filename ends in .gz, wrap in GZIPInputStream
   */
  ...
  /**
   * Open an input stream to a file; do not mask IOExceptions. If the filename is null or "-",
  ...
  public static InputStream openFileEx(String filename) throws IOException {
    ...
    if (filename.endsWith(".gz")) in = new GZIPInputStream(in);
    return in;
  }

  /**
   * Open a UTF8 Reader for a file. If the filename is null or "-", use System.in If the filename
   * ends in .gz, use GZIPInputStream
   */
  ...
  /**
   * Open an ASCII Reader for a file. If the filename is null or "-", use System.in If the filename
   * ends in .gz, use GZIPInputStream
   */
